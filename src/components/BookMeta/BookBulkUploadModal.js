...
import createZipAndDownloadBooks from './createZipAndDownloadBooks';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { DotLoader } from "react-spinners";

const BookBulkUploadModal = ({ closeModal }) => {
  ...
  const downloadSampleTemplate = () => {
    const sampleData = [
      ['title', 'isbn', 'author','genre','subgenre','publishers','noOfCopies']
    ];
  ...
  const handleSubmit =async () => {
    ...
    const response = await globalCon.addBulkBook(validEntries);
    setLoader(false);
    ...
    if(response.return_code === 2){
      ...
      await createZipAndDownloadBooks(response, discardedEntries);
    }
    setLoader(false);
    closeModal();
